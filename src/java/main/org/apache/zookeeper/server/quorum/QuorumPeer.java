...
/**
 * This class manages the quorum protocol. There are three states this server can be in: <ol> <li>Leader election - each server will elect a leader (proposing itself as a leader initially).</li>
 * <li>Follower - the server will synchronize with the leader and replicate any transactions.</li> <li>Leader - the server will process requests and forward them to followers. A majority of followers
 * must log the request before it can be accepted. </ol>
 *
...
public class QuorumPeer extends ZooKeeperThread implements QuorumStats.Provider {
     ...
     *
     * This method resolves hostname for a given server ID in both quorumVerifer and lastSeenQuorumVerifier. If the server ID matches the local server ID, it also updates myQuorumAddr and
     * myElectionAddr.
     */
