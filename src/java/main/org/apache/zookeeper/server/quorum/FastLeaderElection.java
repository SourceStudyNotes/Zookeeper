...
/**
 * Implementation of leader election using TCP. It uses an object of the class QuorumCnxManager to manage connections. Otherwise, the algorithm is push-based as with the other UDP implementations.
 *
 * There are a few parameters that can be tuned to change its behavior. First, finalizeWait determines the amount of time to wait until deciding upon a leader. This is part of the leader election
 * algorithm.
 */
...
public class FastLeaderElection implements Election {
    ...
    /**
     * Upper bound on the amount of time between two consecutive notification checks. This impacts the amount of time to get the system up again after long partitions. Currently 60 seconds.
     */

    final static int maxNotificationInterval = 60000;
    /**
     * This value is passed to the methods that check the quorum majority of an established ensemble for those values that should not be taken into account in the comparison (electionEpoch and zxid).
     */
    ...
    /**
     * Constructor of FastLeaderElection. It takes two parameters, one is the QuorumPeer object that instantiated this object, and the other is the connection manager. Such an object should be created
     * only once by each peer during an instance of the ZooKeeper service.
     *
    ...
    /**
     * This method is invoked by the constructor. Because it is a part of the starting procedure of the object that must be on any constructor of this class, it is probably best to keep as a separate
     * method. As we have a single constructor currently, it is not strictly necessary to have it separate.
     *
     * @param self    QuorumPeer that created this object
     * @param manager Connection manager
     */
    ...
    /**
     * Termination predicate. Given a set of votes, determines if have sufficient to declare the end of the election round. 分析：http://www.cnblogs.com/leesf456/p/6508185.html
     *
    ...
    /**
     * In the case there is a leader elected, and a quorum supporting this leader, we have to check if the leader has voted and acked that it is leading. We need this check to avoid that peers keep
    ...
    /**
     * A learning state can be either FOLLOWING or OBSERVING. This method simply decides which one depending on the role of the server.
     *
     * @return ServerState
     */
    ...
    public Vote lookForLeader() throws InterruptedException {
        ...
        try {
            HashMap<Long, Vote> recv_set = new HashMap<Long, Vote>();

            HashMap<Long, Vote> out_of_election = new HashMap<Long, Vote>();
            ...
            while ((self.getPeerState() == ServerState.LOOKING) && (!stop)) {
                ...
                if (n == null) {
                    ...
                    /*
                     * Only proceed if the vote comes from a replica in the current or next voting view.
                     */
                    switch (n.state) {
                        case LOOKING:
                            /**
     ...
     * or same zxid and higher server id
     */
    ...
    /**
     * Messages that a peer wants to send to other peers. These messages can be both Notifications and Acks of reception of notification.
     */
    ...
    /**
     * Multi-threaded implementation of message handler. Messenger implements two sub-classes: WorkReceiver and  WorkSender. The functionality of each is obvious from the name. Each of these spawns a
     * new thread.
     */
